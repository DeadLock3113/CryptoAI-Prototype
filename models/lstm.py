...
This module implements a Long Short-Term Memory (LSTM) neural network for time series
forecasting of cryptocurrency prices. The LSTM model is designed to capture long-term
dependencies in the price data.
...
class LSTMModel:
    ...
    def __init__(self, sequence_length=60, units=100, dropout_rate=0.2, 
        ...
        logger.info(f"Initialized LSTM model with sequence_length={sequence_length}, units={units}")
    ...
    def train(self, df, target_column='close', epochs=None):
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                filepath=os.path.join(model_dir, 'lstm_model'),
                monitor='val_loss',
                save_best_only=True
            ),
            ...
            # TensorBoard logging
            TensorBoard(log_dir=os.path.join('logs', f'lstm_{datetime.now().strftime("%Y%m%d-%H%M%S")}'))
        ]
    ...
    def evaluate(self, X_test, y_test):
        ...
        # Evaluate model
        logger.info("Evaluating LSTM model on test data")
        ...
        # Log metrics
        logger.info(f"LSTM model evaluation: MSE={metrics['mse']:.4f}, MAE={metrics['mae']:.4f}, RMSE={metrics['rmse']:.4f}")
    ...
    def save(self, filepath='models/saved/lstm_model'):
        """
    ...
    def load(self, filepath='models/saved/lstm_model'):
        """
    ...
    def plot_predictions(self, y_true, y_pred, title="LSTM Model Predictions"):
        """
